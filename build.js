...
const { BASE_URL, SEASON_LABEL, FORM_ENDPOINT, PHONE, CAMPS, COMMON, GRADES, AGE_GROUPS, COUNTRIES, STUDY, STPAUL, ELC, ELC_AUDIENCES, SCHEDULES, CAMP_FAQ } = require("./data.js");
...
<section class="section"><div class="wrap">
  ...
  ${sitePhotos("about")}
  <div class="fit-grid" style="margin-top:18px">
...
/* 밝은 섹션 안의 .btn-line은 흰 바탕에 흰 글씨라 안 보였다(홈 유학·세인트폴·토플면제, 유학 허브·학년별) → 남색 테두리 (2026-09-17) */
...
.cmp thead th,.cmp tbody th{position:relative}
.cmp thead th>a::after,.cmp tbody th>a::after{content:"";position:absolute;inset:0}
.cmp thead th>a:focus-visible{outline:2px solid #fff;outline-offset:2px}
.cmp tbody th>a:focus-visible{outline:2px solid var(--navy);outline-offset:2px}
...
/* 가이드 본문 링크 스타일(.guide-body a: 파랑·밑줄)이 CTA 남색 버튼 글씨를 덮던 것 (2026-09-17) */
.guide-cta .btn{text-decoration:none}
.guide-cta .btn-navy{color:#fff}
...
/* ============================================================
   ...
   ⚠ 이 블록은 CSS 문자열 맨 끝에 둔다 — 앞에 있는 더 좁은 미디어쿼리를
     소스 순서로 이겨야 한다. 데스크톱(≥761px)은 이 블록 밖이라 그대로다.
   ============================================================ */
:root{--sec-y:72px;--head-y:28px;--grid-g:18px;--card-p:24px}
...
@media(max-width:760px){
  ...
  /* 격자 사이 */
  .camp-grid,.card-grid,.grid-2,.grid-3,.step-grid,.guide-grid,.teaser-grid{gap:var(--grid-g)}
  ...
  .faq-list{gap:8px}
  .faq-item summary{padding:13px 28px 13px 0}
  .faq-item p{padding-bottom:14px}

  /* 푸터 — 227장 전부에 똑같이 들어가는 가장 큰 덩어리다(elc.html 기준 2,070px).
     링크 80개는 그대로 두고 알약 사이와 높이만 좁힌다.
     ⚠ 아래 여백은 플로팅 [전화 상담] 버튼을 피하는 값이라 76px 밑으로 내리지 말 것. */
  .site-footer{padding:40px 0 76px}
  .footer-grid{gap:24px}
  .footer-word{margin-bottom:12px}
  .footer-links h3{margin-bottom:10px;font-size:14.5px}
  .footer-linkset{gap:6px 5px}
  /* ⚠ 알약 자체는 줄이지 않는다 — 지금도 29px 이라 더 줄이면 손가락으로 누르기 힘들다.
     좁히는 건 알약 사이 간격까지다. */
  .footer-fine{margin-top:20px;padding-top:14px}
}

@media(max-width:560px){
  :root{--sec-y:42px;--head-y:16px;--grid-g:10px;--card-p:18px}
}
`;
